...
class predefinedDeviceTeachIn {
    ...
    let native = {}
		...
		if(device.EEP.length > 0){
			native.eep = device.EEP
		}
		if(device.imgURL){
		  native.imgURL = device.imgURL
		}
		this.adapter.log.debug(JSON.stringify(device))
    await this.adapter.extendObjectAsync(this.senderID.toLowerCase(), {native: native})
	}
